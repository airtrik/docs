...
$(codeSelectorPrefix+language).parentsUntil(".highlight").show();$(".lang-specific."+language).parentsUntil(".highlight").show();if($(window.location.hash).get(0)){$(window.location.hash).get(0).scrollIntoView(true);}}
...
$(function(){$(".lang-selector a").on("click",function(){var language=$(this).data("language-name");pushURL(language);activateLanguage(language);return false;});window.onpopstate=function(){activateLanguage(getLanguageFromQueryString());};});})(window);;$(function(){loadToc($('#toc'),'.toc-link','.toc-list-h2',10);setupLanguages($('body').data('languages'));$('.content').imagesLoaded(function(){window.recacheHeights();window.refreshToc();});});window.onpopstate=function(){activateLanguage(getLanguageFromQueryString());};;;(function(){var lunr=function(config){var builder=new lunr.Builder
...
var re=new RegExp(pattern,flag);return this.each(function(){jQuery.highlight(this,re,settings.element,settings.className);});};;;(function(){'use strict';var content,searchResults;var highlightOpts={element:'span',className:'search-highlight'};var searchDelay=0;var timeoutHandle=0;var index;$(populate);$(bind);function populate(){index=lunr(function(){this.ref('id');this.field('title',{boost:10});this.field('body')
this.pipeline.add(lunr.trimmer,lunr.stopWordFilter);var that=this;$('h1, h2').each(function(){var title=$(this);var body=title.nextUntil('h1, h2');that.add({id:title.prop('id'),title:title.text(),body:body.text()});});determineSearchDelay(this);})}
function determineSearchDelay(index){if(index._documents.length>5000){searchDelay=300;}}
